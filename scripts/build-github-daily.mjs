...
for (const entry of recentEntries) {
  ...
  const markdownText = await readFile(sourcePath, "utf8");
  ...
  await copyFile(sourcePath, path.join(markdownDirectory, entry.name));
  await writeFile(
...
console.log(`已编译 ${reports.length} 篇日报（${cutoff} — ${entries[0].date}）`);
